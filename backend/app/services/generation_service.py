...
def _plan_model() -> str:
    """The Topic Plan lane (root artifact — see config). Empty setting falls
    through to the heavy generation model."""
    try:
        from app.core.config import settings
        return settings.generation_plan_model or settings.generation_model
    except Exception:  # pragma: no cover - settings unavailable offline
        return MODEL
...
def run_topic_job(db, job_id: str, course_id: str, topic_id: str) -> None:
    ...
    def attempt_plan() -> tuple[dict, ValidationResult | None]:
        try:
            p_ = ingest_topic_plan(gen_topic_plan(client, ctx))
            p_ = repair_plan_alignment(client, p_)
        except Exception as e:
            logger.warning("topic plan attempt parse/gen failed: %s", e)
            return {}, None
        r = validate_topic_plan(p_, credits=ctx.get("credits"))
        log_check_outcomes(job_id, "topic_plan", r, db=db)   # telemetry from day one (§6)
        return p_, r

    # The plan runs on its own lane (see generation_plan_model) — light-lane
    # repairs inside repair_plan_alignment are unaffected by the override.
    plan_model = _plan_model()
    plan: dict = {}
    result: ValidationResult | None = None
    _ovr = _MODEL_OVERRIDE.set(plan_model)
    try:
        for _attempt in range(1, _MAX_ATTEMPTS + 1):
            plan, result = attempt_plan()
            if result and result.all_pass:
                break
    finally:
        _MODEL_OVERRIDE.reset(_ovr)

    # Same escalation contract as concept artifacts: one rerun on the
    # escalation model when the retry cap is exhausted with blocking failures.
    # Moot while the plan lane is already the escalation tier; it matters when
    # generation_plan_model is pointed back at a cheaper model.
    esc = _escalation_model()
    if (not result or not result.all_pass) and esc and esc != plan_model:
        logger.warning("topic plan gates failed on %s for topic %s — escalating to %s",
                       plan_model, topic_id, esc)
        _ovr = _MODEL_OVERRIDE.set(esc)
        try:
            esc_plan, esc_result = attempt_plan()
        finally:
            _MODEL_OVERRIDE.reset(_ovr)
        def _blocking(r: ValidationResult | None) -> int:
            return sum(1 for c in r.failures if c.blocking) if r else 99
        if esc_result and (esc_result.all_pass or _blocking(esc_result) <= _blocking(result)):
            plan, result, plan_model = esc_plan, esc_result, esc
    ...
    if not result or not result.all_pass:
        ...
        _upsert_artifact(db, job_id, topic_id, "topic_plan", plan,
                         review_status="revise_requested", model_used=plan_model,
    ...
    _upsert_artifact(db, job_id, topic_id, "topic_plan", plan,
                     ...
                     token_count=p_tok + c_tok, cost_usd=cost, model_used=plan_model)
...
def estimate_topic_cost(plan: dict, grounded: bool = False) -> float:
    ...
    total = ct.estimate_artifact_cost("topic_plan", _plan_model()) + n * per_concept + topic_arts
    if grounded:
...
def generate_topic_artifact(db, job_id, topic_id, artifact_type) -> None:
    ...
    model_used = _model()
    esc = _escalation_model()
    # Escalate only on a recorded failing verdict — most topic artifacts carry
    # no deterministic gate, and an absent validation must not read as failure.
    if (content.get("validation") or {}).get("all_pass") is False and esc and esc != model_used:
        logger.warning("blocking gates failed on %s for topic artifact %s — escalating to %s",
                       model_used, artifact_type, esc)
        _ovr = _MODEL_OVERRIDE.set(esc)
        try:
            retry = run_once()
        finally:
    ...
    _upsert_artifact(db, job_id, topic_id, artifact_type, content,
                     ...
                     token_count=p_tok + c_tok, cost_usd=cost, model_used=model_used)
    _add_job_cost(db, job_id, p_tok + c_tok, cost)
