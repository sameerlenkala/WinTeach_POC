...
get_db returns a service-role client that bypasses RLS, so every mutating
route guards course ownership explicitly (faculty own their courses; admins
their institute; superadmin anything) — same posture as library_service.
"""
...
def _guard_course(user: dict, course: dict) -> None:
    ...
    if role == "admin" and course.get("institute_id") == user.get("institute_id"):
        return
    if role == "faculty" and course.get("faculty_id") == user["id"]:
        return
...
async def upload_material(
    ...
    try:
        ...
        pass  # Storage optional in local dev (same as syllabus uploads)
...
def list_my_materials(user: dict = Depends(_faculty_above), db: Client = Depends(get_db)):
    ...
    out = []
    for r in (q.order("created_at", desc=True).execute().data or []):
...
def get_material_chunks(ids: str, user: dict = Depends(get_current_user),
                        db: Client = Depends(get_db)):
    ...
    return [{**r, "filename": (r.pop("materials", None) or {}).get("filename")}
            for r in rows]


@router.get("/materials/{material_id}")
def get_material(material_id: str, user: dict = Depends(get_current_user),
                 db: Client = Depends(get_db)):
    return _material_or_404(db, material_id)
...
def download_material(material_id: str, user: dict = Depends(get_current_user),
                      db: Client = Depends(get_db)):
    ...
    return Response(
        ...
        headers={"Content-Disposition": f'inline; filename="{material.get("filename", "material")}"'},
    )
...
def _course_materials_with_topics(db: Client, course_id: str) -> list[dict]:
    ...
    rows = (db.table("materials").select("*, topic_materials(topics(id,title))")
...
def list_course_materials(course_id: str, user: dict = Depends(get_current_user),
                          db: Client = Depends(get_db)):
    return _course_materials_with_topics(db, course_id)


@router.get("/topics/{topic_id}/materials")
def list_topic_materials(topic_id: str, user: dict = Depends(get_current_user),
                         db: Client = Depends(get_db)):
