...
import { useInstitutes } from '@/api/hooks';
...
function RolePill({ role }: { role: string }) {
  const map: Record<string, { bg: string; color: string }> = {
    superadmin: { bg: T.brandBg, color: T.brand },
    admin:      { bg: 'rgba(91,75,219,0.10)', color: '#5B4BDB' },
  ...
  return (
    <span style={{ padding: '3px 10px', borderRadius: 48, fontSize: 12, fontWeight: 600, ...s }}>
      {role === 'admin' ? 'College Admin' : role === 'superadmin' ? 'Super Admin' : role.charAt(0).toUpperCase() + role.slice(1)}
    </span>
  );
}
...
export default function SuperAdminUsers() {
  ...
  const needsCollege = role === 'admin' || role === 'faculty';
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setFullName(''); setEmail(''); setPassword(''); setInstId('');
    } catch (e: any) {
      ...
      <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: T.r5, padding: 28, marginBottom: 24, boxShadow: T.shadow }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
          ...
          <div>
            ...
            <input value={email} onChange={e => setEmail(e.target.value)} placeholder="admin@college.edu" style={inp} />
          </div>
          {/* Role */}
          <div>
          ...
          {needsCollege && (
            <div>
              ...
              <select value={instId} onChange={e => setInstId(e.target.value)} style={inp}>
