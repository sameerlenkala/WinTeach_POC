...
function ConceptTile({ jobId, conceptId, type, state, locked, onChanged, onView }: {
  ...
  // Publish floor mirror: the backend 409s approval when blocking gates failed
  // (even after terra escalation) — hide the button rather than surface the 409.
  const gateFailed = state?.gate_passed === false;
  // Model provenance: short tag, highlighted when the artifact needed escalation
  // off the default lane (e.g. terra rescuing a luna gate failure).
  const modelShort = state?.model_used?.replace(/^gpt-[\d.]+-/, '') ?? null;
  const escalated = modelShort != null && modelShort !== 'luna';
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', gap: 6, marginTop: 10, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        {status === 'ready' && <>
          ...
          {!approved && !gateFailed && <Btn sm variant="primary" onClick={approve}>Approve</Btn>}
        ...
        {(modelShort || (state?.cost_usd ?? 0) > 0) && (
          <span style={{ marginLeft: 'auto', display: 'inline-flex', gap: 6, alignItems: 'center' }}>
            {modelShort && (
              <span
                title={escalated
                  ? `Escalated: gates failed on the default model, regenerated on ${state?.model_used}`
                  : `Generated by ${state?.model_used}`}
                style={{
                  fontSize: 10.5, borderRadius: 5, padding: '1px 6px',
                  color: escalated ? W.orangeFg : W.text3,
                  background: escalated ? 'color-mix(in oklab, var(--status-orange) 10%, var(--card))' : W.surfaceMuted,
                  border: `1px solid ${escalated ? 'color-mix(in oklab, var(--status-orange) 35%, transparent)' : W.border}`,
                }}>
                {modelShort}
              </span>
            )}
            {(state?.cost_usd ?? 0) > 0 && <span style={{ fontSize: 11, color: W.text3, fontVariantNumeric: 'tabular-nums' }}>{usd(state?.cost_usd)}</span>}
          </span>
        )}
      </div>
      {showGrounding && (
